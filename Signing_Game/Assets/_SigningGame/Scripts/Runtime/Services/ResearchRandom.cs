...
namespace Services {
    internal interface IResearchRandom {
        ulong State { get; }
        int NextInt(int minimumInclusive, int maximumExclusive);
        bool Chance(double probability);
        IResearchRandom Fork();
    }

    internal sealed class ResearchRandom : IResearchRandom {
        private ulong _state;
        public ulong State => _state;

        public ResearchRandom(ulong state) => _state = state;
        ...
        public IResearchRandom Fork() => new ResearchRandom(_state);
