...
namespace SigningGame.Editor.Modifiers {
    [CustomPropertyDrawer(typeof(NumericValueDefinition), true)]
    internal sealed class NumericValueDefinitionPropertyDrawer : PropertyDrawer {
        private const string _SelectorPlaceholder = "Select numeric value...";
        ...
        private static Candidate[] DiscoverCandidates() {
            ...
            var candidates = TypeCache.GetTypesDerivedFrom<NumericValueDefinition>()
        ...
        private static bool IsCandidate(Type type, ISet<string> playerAssemblyNames) {
            if (type == null
                || !typeof(NumericValueDefinition).IsAssignableFrom(type)
                || !type.IsClass
                || !type.IsSerializable
                || type.IsAbstract
                || type.IsGenericType
        ...
        private static bool TryClearMissing(SerializedObject serializedObject, string propertyPath) {
            ...
            try {
                ...
                Undo.RegisterCompleteObjectUndo(targetObject, "Clear Missing Numeric Value Reference");
